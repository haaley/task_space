...
import 'package:task_scape/views/actions_pages/edit_guests_widgets/addOutsideGuest.dart';
...
class NewGuestEdit extends StatefulWidget {
  final self;
  final id;
  final startDate;
  final endDate;

  const NewGuestEdit({Key key, this.self, this.id, this.startDate, this.endDate}) : super(key: key);

  @override
  _NewGuestEditState createState() => _NewGuestEditState();
}

class _NewGuestEditState extends State<NewGuestEdit>  with SingleTickerProviderStateMixin {
  ...
  void loadData() async {
    await UserViewModel.loadUsers(widget.startDate, widget.endDate);
    setState(() {
    });
  }
  ...
  Widget buildList(context, index) {
    return GestureDetector(
        ...
        onTap: () async{
          await NewMeetingPageController.instance.addGuestEdit(context, users[index].fullname, users[index].email,
              users[index].id, users[index].conflict, MeetingPageController.instance.add_guest);
          _filter.text = "";
        });
  }
}
